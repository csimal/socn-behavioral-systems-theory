...
Computing the Most Powerful Unfalsified Model $\B_{MPUM}(w_d)$ of a finite trajectory $w_d$ is fairly straightforward. From a candidate $l$ between $1$ and $\ell_{\max} = \lfloor \frac{T+1}{q+1} \rfloor$, we estimate the corresponding $m$ and $n$ from $\rank \mathscr{H}_l (w_d)$ and $\rank \mathscr{H}_{l+1}(w_d)$, and iterate over all possible values of $l$ to find the minimizer. This feature is available through the function \codeinline{complexity_mpum}.
